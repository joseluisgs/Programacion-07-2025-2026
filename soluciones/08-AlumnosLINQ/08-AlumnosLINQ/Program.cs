...
var todosAlumnos = from a in listaAlumnos
                   select a;
todosAlumnos.ForEach(WriteLine);
...
notaMediaPorCurso.ForEach(g => WriteLine($"{g.Curso}: {g.Media:F2}"));
...
estadisticasPorCurso.ForEach(g => 
...
cursosConMasDe3.ForEach(g => WriteLine($"{g.Curso}: {g.Cantidad} alumnos"));
...
var cursosNotaAlta = from a in listaAlumnos
                     ...
                     select new { Curso = grupo.Key, Media = grupo.Average(a => a.Nota) };
cursosNotaAlta.ForEach(g => WriteLine($"{g.Curso}: Media = {g.Media:F2}"));
...
var pagina3 = (from a in listaAlumnos
               select a).Skip(10).Take(5).ToList();
...
mejorPorCursoProyeccion.ForEach(m => WriteLine($"{m.Curso}: Mejor={m.MejorAlumno} ({m.MejorNota})"));
